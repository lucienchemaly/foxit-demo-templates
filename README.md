...
| [`remittance_confirmation.docx`](remittance_confirmation.docx) | Remittance advice confirming payment of an invoice | `{{ payer_name }}`, `{{ remittance_id }}`, `{{ invoice_number }}`, `{{ payment_date }}`, `{{ payment_method }}`, `{{TableStart:line_items}}` / `{{TableEnd:line_items}}` loop with `{{ROW_NUMBER}}` / `{{description}}` / `{{qty}}` / `{{total \# "$#,##0.00"}}`, plus `{{ amount_paid \# "$#,##0.00" }}` | Drop-in template for the batch4 article5 agentic invoice pipeline (Step 5, DocGen remittance generation) |
